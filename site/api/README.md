...
Wszystkie dane są wysyłane i odbierane jako JSON. Puste pola są uwzględniane jako puste, a nie pomijane.
...
Uwierzytelnienie *Użytkownika* może odbywać się kilkoma metodami, a celem jest pozyskanie w każdym przypadku identyfikatora sesji *Użytkownika*. 
...
W celu wykorzystania SSH do uwierzytelniania należy uzyskać połączenie SSH z *Platformą*, która wówczas zwróci identyfikator sesji *Użytkownika*:
...
W celu oczekiwania na wykonanie operacji przez określony czas należy przesłać nagłówek w nagłówku żądania:
...
Warto przy tym wykorzystać [parametr ```--verbose```](/h1-cli/common-arguments.md) CLI. W celu uzyskania informacji o odpowiedzi należy wykorzystać [parametry ```-o json```](/h1-cli/common-arguments.md). Przykładowo:
