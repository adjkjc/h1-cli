# TOC
  ...
  * [h1 container wait_boot](#h1-container-wait_boot) - Wait to boot container
...
## h1 container wait_boot

Wait to boot container

### Syntax

```h1 container wait_boot | --container CONTAINER [--tag TAG [--tag TAG ...]]```
### Example

```bash
h1 container wait_boot --name my-server-log
```
...
```h1 container create | --name NAME --image IMAGE --type TYPE [--env ENV [--env ENV ...]] [--entrypoint ENTRYPOINT] [--args ARGS] [--os-size OS-SIZE] [--data-disk DATA-DISK] [--log LOG] [--log-password LOG-PASSWORD] [--mountpoint MOUNTPOINT [--mountpoint MOUNTPOINT ...]] [--ip IP] [--tag TAG [--tag TAG ...]]```
### Example
...
| ```--args ARGS``` |  | Extra arguments on run of image |
| ```--os-size OS-SIZE``` |  | Size of OS disk. Default: 10 GB |
| ```--data-disk DATA-DISK``` |  | Data disk: "type,size" or "id" |
| ```--log LOG``` |  | Log ID or name |
| ```--log-password LOG-PASSWORD``` |  | Log ID or name |
