...
const getCommon = async (type = 'a1.nano') => {
    const token = await tests.getToken();
    const vm_name = `vm-test-${now}`;
    const disk_name = `disk-${vm_name}`;
    const image = await tests.getImage();
    const params = `--type ${type} --password ${token} --name ${vm_name} --image ${image._id} --os-disk ${disk_name},ssd,10`;
    return {
        params: params,
        disk_name: disk_name,
        name: vm_name,
...
ava.test.serial('vm disk attach & detach', async t => {
    const common = await getCommon();
    const vm = await tests.run(`vm create ${common.params}`);
    const disk = await tests.run(`disk create --name disk-${common.name} --type ssd --size 10`);
...
ava.test.serial('vm nic life cycle', async t => {
    ...
    await tests.resourceLifeCycle('vm nic', `--vm ${vm._id} --type private --network ${network._id}`)(t);
    await tests.resourceLifeCycle('vm nic', `--vm ${vm._id} --type public`)(t);
...
ava.test.serial('vm nic ip life cycle', async t => {
    const common = await getCommon('m2.tiny');
    const vm = await tests.run(`vm create ${common.params}`);
...
ava.test.serial('vm serialport log', async t => {
    ...
    const content = await tests.run(`vm serialport log --vm ${vm._id}`);
    t.true(content.length > 10);
