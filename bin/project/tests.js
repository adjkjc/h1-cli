...
const TEST_ACCESS_EMAIL = 'h1-testing@jawnosc.tk';
// const TEST_ACCESS_TENANT = '5b28f7c9ddd5b3c5e9ec14c4';
...
['user', 'owner', 'billing'].forEach(role => {
    ava.test.serial(`project access ${role} cycle`, async t => {
        ...
        for (const action of actions) {
            await tests.run(`project access ${action.name} --email ${TEST_ACCESS_EMAIL} --project ${project} ${action.params || ''}`);
            const list = await tests.run('project access list');
            t.true(list.some(d => d.role === role && d.id === TEST_ACCESS_EMAIL) === action.result);
        }
    });
});
