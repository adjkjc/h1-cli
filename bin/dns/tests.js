...
const queryNameserver = (t, hostname, rrtype, nss) => new Promise(async (resolve, reject) => {
    const socket = dnsSocket();
    t.log(`Query DNS ${nss} about ${rrtype} ${hostname}`);
...
tests.serial('dns zone life cycle', ['dns'], tests.resourceLifeCycle('dns zone', {
...
tests.serial('dns zone export', ['dns'], async t => {
    const name = `dns-export-${now}.com`;
    const zone = await tests.run(t, `dns zone create --type public --name ${name}`);
    const export_content = await tests.run(t, `dns zone export --zone ${name}`);
    t.true(export_content.includes('hostmaster.hyperone.com.'));
    await tests.remove(t, 'dns zone', zone);
});
...
const test_record_values = async (t, zone, type, name, expected_values) => {
    const list = await tests.run(t, `dns record-set ${type} list --zone ${zone.dnsName}`);
...
Object.entries(recordTypes).forEach(([type, { expected, values }]) => {
    tests.serial(`dns record-set ${type} life cycle`, ['dns'], async t => {
        const zone = await tests.run(t, `dns zone create --type public --name ${type}.dns-record-set-${now}.com`);
        const name = `${type}-${now}`;
        const full_name = `${name}.${zone.dnsName}`;

        // Create record-set
        await tests.run(t, `dns record-set ${type} create --name ${name} --zone ${zone.dnsName} --value '${values[0]}'`);
        await test_record_values(t, zone, type, full_name, [values[0]]);

        if (values.length > 1) {
            // Add record to record-set
            await tests.run(t, `dns record-set ${type} add-record --name ${name} --zone ${zone.dnsName} --value '${values[1]}'`);
            await test_record_values(t, zone, type, full_name, [values[0], values[1]]);

            // Delete record from record-set
            await tests.run(t, `dns record-set ${type} delete-record --name ${name} --zone ${zone.dnsName} --value '${values[1]}'`);
            await test_record_values(t, zone, type, full_name, [values[0]]);

            // Upsert record-set
            await tests.run(t, `dns record-set ${type} upsert --name ${name} --zone ${zone.dnsName} --value '${values[0]}'`);
            await test_record_values(t, zone, type, full_name, [values[0]]);
        }
        // Remove record-set at all
        await tests.run(t, `dns record-set ${type} delete --name ${name} --zone ${zone.dnsName}`);
        const list = await tests.run(t, `dns record-set ${type} list --zone ${zone.dnsName}`);
    ...
    tests.serial(`dns record-set ${type} response`, ['dns'], async t => {
        const zone = await tests.run(t, `dns zone create --type public --name ${type}.dns-record-set-${now}.com`);
        try {
            const name = `${type}-${now}`;
            const full_name = `${name}.${zone.dnsName}`;
    ...
    tests.serial(`dns exports-import record type ${type}`, ['dns'], async t => {
        const zone = await tests.run(t, `dns zone create --type public --name ${type}.export-${now}.com`);
        const zone_import = await tests.run(t, `dns zone create --type public --name ${type}.import-${now}.com`);
        try {
            const name = `${type}-${now}`;

            const recordset_start = await tests.run(t, `dns record-set ${type} list --zone ${zone.dnsName}`);
            const zone_start_content = await tests.run(t, `dns zone export --zone ${zone.dnsName}`);
            const argv = values.map(x => `--value '${x}'`);
            await tests.run(t, `dns record-set ${type} create --name ${name} --zone ${zone.dnsName} ${argv.join(' ')}`);
            // Validates that export works
            const content = await tests.run(t, `dns zone export --zone ${zone.dnsName}`);
            ...
            await tests.run(t, `dns zone import --zone ${zone_import.name} --zone-file ${zone_file}`);

            // Validates that import works via validated export
            const content_export = await tests.run(t, `dns zone export --zone ${zone_import.name}`);
            for (const value of values) {
                t.true(content_export.includes(value.split(/\s+/)[0].replace(/ /g, '\t')));
            }
            const recordset_imported = await tests.run(t, `dns record-set ${type} list --zone ${zone.dnsName}`);
            ...
            await tests.run(t, `dns zone import --zone ${zone_import.name} --zone-file ${zone_start_file} --delete`);
            const recordset_end = await tests.run(t, `dns record-set ${type} list --zone ${zone_import.name}`);
            t.true(recordset_start.length === recordset_end.length);

            await fsPromiseUnlink(zone_file);
        } finally {
            await tests.remove(t, 'dns zone', zone);
            await tests.remove(t, 'dns zone', zone_import);
        }
    });
});

tests.serial('import zone from file', ['dns'], async t => {
...
`);
    ...
    const zone = await tests.run(t, `dns zone create --type public --name import-file-${now}.com`);
    await tests.run(t, `dns zone import --zone ${zone.dnsName} --zone-file ${zone_file}`);
...
tests.serial('dns record-set a dynamic-dns', ['dns'], async t => {
    const zone = await tests.run(t, `dns zone create --name ddns-${now}.com --type public`);
    const rrset = await tests.run(t, `dns record-set a dynamic-dns --name my-home-${now} --zone ${zone.dnsName}`);
...
tests.serial('dns manage record-set after rename', ['dns'], async t => {
    const zone = await tests.run(t, `dns zone create --type public --name A.dns-record-set-${now}.com`);
    await tests.run(t, `dns zone rename --zone ${zone.id} --new-name 'renamed.com.'`);
    const name = `my-name.${zone.dnsName}`;
    const rrset = await tests.run(t, `dns record-set a create --name ${name} --zone ${zone.id} --value '1.1.1.1'`);
    t.true(rrset.name == name);
    await tests.remove(t, 'dns zone', zone);
});

tests.serial('dns responds on wildcard requests', ['dns'], async t => {
    const value = '2.2.2.2';
    const zone = await tests.run(t, `dns zone create --type public --name A.dns-record-set-${now}.com`);
    await tests.run(t, `dns record-set a create --name '*.wildcard' --zone ${zone.id} --value ${value}`);
...
tests.serial('dns resolve cname at apex', ['dns'], async t => {
    const ip = '2.2.2.2';

    const content = await tests.getToken();
    const zone = await tests.run(t, `dns zone create --type public --name ${tests.getName(t.title)}.com`);

    await tests.run(t, `dns record-set cname create --name '@' --zone ${zone.id} --value ${ip}.xip.io`);
    await tests.run(t, `dns record-set txt create --name '@' --zone ${zone.id} --value ${content}`);
