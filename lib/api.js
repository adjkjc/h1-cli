...
exports.getApiKey = async (user, body) => {
    try {
        const rsp = await api('post', `user/${user}/session`)
            .send(body);
...
exports.getApiKeySSH = async user => {
...
exports.redirect_url = async (uri, query) => {
    const resp = await exports
        .head(uri, {})
        .query(query)
        .redirects(0)
