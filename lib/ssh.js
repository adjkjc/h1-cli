...
const execute = (t, command, connection) => new Promise((resolve, reject) =>
    getConnection(connection).then(conn => {
        const buffer = [];
        t.log(`[${connection.host}]:${command}`);
...
const execResource = (t, resource, auth, cmd) => {
    t.log(new Date().toISOString(), `[resource: ${resource.id}]`, cmd);
...
const execVm = (t, resource, auth, cmd) => execResource(t, resource, Object.assign({username: 'guru'}, auth), cmd);
