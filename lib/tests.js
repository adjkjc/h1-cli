...
const openApiValidator = async (t, schemaRef, data) => {
    t.log(new Date().toISOString(), `Validate match against ${schemaRef}`);
...
const parallel = (title, features = [], func) => {
    const missing = features
        .filter(x => !scopes[process.env.SCOPE_NAME].features.includes(x));
    if (missing.length > 0) {
        return ava.skip(title, `Missing features: ${missing.join(',')}`);
    }
    return ava(title, func);
};

const serial = (title, features = [], func) => {
    const missing = features
        .filter(x => !scopes[process.env.SCOPE_NAME].features.includes(x));
    if (missing.length > 0) {
        return ava.skip(title, `Missing features: ${missing.join(',')}`);
    }
    return ava.serial(title, func);
};

const delay = (t, timeout) => new Promise(resolve => {
    console.log(t, `Delay ${timeout / 1000} seconds`);
...
const run = async (t, cmd) => {
    ...
    t.log(new Date().toISOString(), shell_quote.quote(process.argv));
...
const show = async (t, type, resource, options = {}, history = []) => {
...
const remove = async (t, type, resource, options = {}, history = []) => {
    let resource_id = resource;
    if (typeof resource_id === 'object') {
        resource_id = resource_id.id;
    }
...
const wrap = (type, options = {}, func) => async t => {
    ...
    if (options.resource) {
        ...
        resource = await run(t, `${type} ${options.createCommand || 'create'} ${options.createParams}`);
    }
    try {
        await func(t, resource);
    } finally {
        if (!options.skipDelete) {
            await remove(t, type, resource, options);
        }
    }
};
...
const subresourceLifeCycle = (options = {}) => async t => {
    ...
    const sub_list = await run(t, `${categoryName} list ${options.listParams || ''} ${commonParams}`);
    ...
    const subresource = await run(t, `${categoryName} show ${options.showParams || ''} ${commonParams} --${getOption(options.type)} ${last_resource._id}`);
    t.true(last_resource._id === subresource._id);

    if (!options.skipRename) {
        const new_name = `renamed-${subresource.name}`;
        await run(t, `${categoryName} rename ${commonParams} --${getOption(options.type)} ${subresource._id} --new-name '${new_name}' ${options.renameParams || ''}`);
        const renamed = await run(t, `${categoryName} show ${options.showParams || ''} --${getOption(options.type)} ${subresource._id}`);
...
const resourceLifeCycle = (type, options = {}) => wrap(type, options, async (t, resource) => {
    ...
    if (!options.skipHistory) {
        const history = await run(t, `${type} history ${options.historyParams || ''} --${getOption(type)} '${resource.name}'`);
    ...
    if (options.delayStateTest) {
        await delay(t, options.delayStateTest);
    }
    const resource_by_id = await show(t, type, resource, options, history);
    ...
    if (!options.skipOpenApi) {
        await openApiValidator(t, options.schemaRef, resource_by_id);
    }
    ...
    if (!options.skipTag) {
        // Add of tag
        await run(t, `${type} tag add ${options.tagParams || ''} --${getOption(type)} '${resource.name}' --tag protected=${now}`);
        const tag_obj = await run(t, `${type} tag list ${options.tagParams || ''} --${getOption(type)} '${resource.name}'`);
        ...
        // Delete of tag
        await run(t, `${type} tag delete --${getOption(type)} '${resource.name}' ${options.tagParams || ''} --tag protected`);
        const tag_obj_new = await run(t, `${type} tag list ${options.tagParams || ''} --${getOption(type)} '${resource.name}'`);
    ...
    await show(t, type, resource.name, options, history);
});
...
const credentialsLifeCycle = (type, options = {}) => async t => {
    ...
    try {
        ...
        const name = `test-${now}`;
        await run(t, `${type} add --name ${name} ${createParams} --sshkey-file '${sshFilename}'`);

        const credentials_list = await run(t, `${type} list ${listParams}`);
        t.true(credentials_list.some(d => d.name === name));

        const credentials = await run(t, `${type} show ${showParams} --${getOption(type)} ${name}`);

        await openApiValidator(t, '#/components/schemas/credential.certificate', credentials);

        await run(t, `${type} rename --${getOption(type)} ${name} --new-name renamed-${name} ${renameParams}`);
        await run(t, `${type} show ${showParams} --${getOption(type)} ${credentials._id}`);

        await remove(t, type, credentials, options);
    } finally {
...
const passwordLifeCycle = async (t, type, resource) => {
    ...
    await run(t, `${type} credential password add --name ${name} --password ${secret} --${type} ${resource.name}`);
    const list = await run(t, `${type} credential password list --${getOption(type)} ${resource.name}`);
    t.true(list.some(p => p.name === name));

    const password = await run(t, `${type} credential password show --${getOption(type)} ${resource.name} --password ${name}`);

    await openApiValidator(t, '#/components/schemas/credential.password', password);

    await run(t, `${type} credential password rename --${getOption(type)} ${resource.name} --password ${name} --new-name ${new_name}`);
    const renamed_password = await run(t, `${type} credential password show --${getOption(type)} ${resource.name} --password ${password._id}`);
    t.true(renamed_password.name === new_name);

    await remove(t, `${type} credential password`, password, {
...
const resourceResizeCycle = (type, options) => async t => {
    ...
    const resource = await run(t, `${type} create ${createParams} --size ${initialSize}`);
    t.true(resource.size === initialSize);

    await run(t, `${type} resize --${type} ${resource._id} --size ${targetSize}`);

    const resized_resource = await show(t, type, resource);
    t.true(resized_resource.size === targetSize);

    await remove(t, type, resource);
};
...
const accessRuleLifeCycle = async (t, type, options) => {
    ...
    const name = `rule-${now}`;
    const resource = await run(t, `${type} add ${options.commonParams || ''} --name ${name} --type geo --value PL`);

    const tester = x => x._id === resource._id;
    const list = await run(t, `${type} list ${options.commonParams || ''}`);
    t.true(list.some(tester));

    const resource_show = await run(t, `${type} show ${options.commonParams || ''} --rule ${resource._id}`);

    await openApiValidator(t, '#/components/schemas/credential.password', resource_show);

    await run(t, `${type} delete ${options.commonParams || ''} --yes --rule ${resource._id}`);
    const updated_list = await run(t, `${type} list ${options.commonParams || ''}`);
...
const requireSlaveProject = (fn) => async (t, ...args) => {
    ...
    try {
        ...
        await run(t, `project select --project '${projects.master}'`); // Just in case, select the project again.
    }
};
...
const resourceAccessOfUser = (type, accessParams) => async t => {
    for (const role of ['user', 'owner', 'billing']) {
        ...
        for (const action of actions) {
            const access = await run(t, `${type} access ${action.name} --email ${RECIPIENT.user} ${accessParams} ${action.params || ''}`);
            await openApiValidator(t, '#/components/schemas/accessrightsUserRole', access);

            const list = await run(t, `${type} access list ${accessParams}`);
