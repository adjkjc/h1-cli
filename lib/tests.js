...
const subresourceLifeCycle = (options = {}) => async t => {
    ...
    const subresource = await run(t, `${categoryName} show ${options.showParams || ''} ${commonParams} --${getOption(options.type)} ${last_resource.id}`);
    t.true(last_resource.id === subresource.id);

    if (!options.skipRename) {
        const new_name = `renamed-${subresource.name}`;
        await run(t, `${categoryName} rename ${commonParams} --${getOption(options.type)} ${subresource.id} --new-name '${new_name}' ${options.renameParams || ''}`);
        const renamed = await run(t, `${categoryName} show ${options.showParams || ''} --${getOption(options.type)} ${subresource.id}`);
...
const credentialsLifeCycle = (type, options = {}) => async t => {
    ...
    try {
        ...
        await run(t, `${type} show ${showParams} --${getOption(type)} ${credentials.id}`);
...
const passwordLifeCycle = async (t, type, resource) => {
    ...
    const renamed_password = await run(t, `${type} credential password show --${getOption(type)} ${resource.name} --password ${password.id}`);
...
const resourceResizeCycle = (type, options) => async t => {
    ...
    await run(t, `${type} resize --${type} ${resource.id} --size ${targetSize}`);
...
const accessRuleLifeCycle = async (t, type, options) => {
    ...
    const resource_show = await run(t, `${type} show ${options.commonParams || ''} --rule ${resource.id}`);

    await openApiValidator(t, '#/components/schemas/credential.password', resource_show);

    await run(t, `${type} delete ${options.commonParams || ''} --yes --rule ${resource.id}`);
